...
bool check_constraint(int iteration, enum constraint con, char former[], int n, int m){
...
void generate_sequence(enum constraint con, int length, float miss_probability, int n, int m){
    if (length < m) 
        printf("! Warning ! \nLength of sequence is shorter than m, any sequence would be valid \n");
    char res[length + m]; // problem
...
void positionController(float* thrust, attitude_t *attitude, setpoint_t *setpoint,
                                                             const state_t *state)
{
// new, own, code here including last line of function (velocityController(thrust, attitude, setpoint, state);)

// bypass unused variable warning -> error
if (velMaxOverhead == 0.00f)
  return;
if (!init){
    init_sequence();
}
